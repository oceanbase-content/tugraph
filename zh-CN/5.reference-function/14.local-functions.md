...
OC#oc#oc#说明lsgetinfolsgetinfolsEnvironmentVariableGet 
-------------------------------------------------
...
```javascript
sword OC#oc#oc#说明lsgetinfolsgetinfolsEnvironmentVariableGet ( void       *val,
size_t     size,
ub2        item,
ub2        charset, 
size_t     *rsize );
...
|      参数      |                                                                                                                     描述                                                                                                                      |
|--------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| val (#oc#oc#说明lsgetinfolsgetinfo/OUT) | 返回全球化支持环境变量的值，例如 `NLS_LANG` 字符集 ID 或 `NLS_NCHAR` 字符集 ID。                                                                                                                                                                                    |
| size (#oc#oc#说明lsgetinfolsgetinfo)    | 指定给定输出值的大小，仅适用于字符串数据。每条信息的最大长度为 `OCI_NLS_MAXBUFSZ` 字节。对于数值数据，此参数将被忽略。                                                                                                                                                                       |
| item (#oc#oc#说明lsgetinfolsgetinfo)    | 从全球化支持环境变量中获取以下指定值之一： * `OCI_NLS_CHARSET_ID`：`ub2` 数据类型的 `NLS_LANG` 字符集 ID。   * `OCI_NLS_NCHARSET_ID`：`ub2` 数据类型的 `NLS_NCHAR` 字符集 ID。    |
| charset (#oc#oc#说明lsgetinfolsgetinfo) | 指定检索到的字符串数据的字符集 ID。如果为 `0`，则使用 `NLS_LANG` 值。`OCI_UTF16ID` 是此参数的有效值。对于数值数据，此参数将被忽略。                                                                                                                                                          |
...
OC#oc#oc#说明lsgetinfolsgetinfolsGetInfo 
----------------------------------
...
```javascript
sword OC#oc#oc#说明lsgetinfolsgetinfolsGetInfo ( void        *hndl, 
OCIError    *errhp, 
OraText     *buf, 
size_t      buflen, 
ub2         item );
...
|       参数       |                                                                                                                                            描述                                                                                                                                            |
|----------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| hndl (#oc#oc#说明lsgetinfolsgetinfo/OUT)  | 以对象模式初始化的 OCI 环境或用户会话句柄。                                                                                                                                                                                                                                                                 |
| errhp (#oc#oc#说明lsgetinfolsgetinfo/OUT) | 错误信息句柄。如果有错误，则记录在 `errhp` 中，并且该函数返回一个 `NULL` 指针。通过调用 [OCIErrorGet](/zh-CN/5.reference-function/7.miscellaneous-functions.md) 获取诊断信息。                                                                                                                                                                          |
| buf (OUT)      | 指向目标缓冲区的指针。返回的字符串以 `NULL` 字符结尾。                                                                                                                                                                                                                                                          |
| buflen (#oc#oc#说明lsgetinfolsgetinfo)    | 目标缓冲区的大小。每条信息的最大长度为 `OCI_NLS_MAXBUFSZ` 字节。 `OCI_NLS_MAXBUFSIZE`：调用 [OC#oc#oc#说明lsgetinfolsgetinfolsGetInfo](#oc#oc#说明lsgetinfolsgetinfo) 时，必须分配缓冲区来存放返回的信息。缓冲区大小取决于您正在查询的项目以及您用于存储信息的编码。该 `OCI_NLS_MAXBUFSZ` 属性保证缓冲区足够大以容纳 [OC#oc#oc#说明lsgetinfolsgetinfolsGetInfo](#oc#oc#说明lsgetinfolsgetinfo) 返回的最大项。 |
| item (#oc#oc#说明lsgetinfolsgetinfo)      | 指定将要返回的 OCI 环境句柄中的项目。其具体取值可参考本小节 [说明](#sectiondiv-aw0-8nh-5c4)。                                                                                                                                                                                                          |



**返回值** ：成功时返回 OCI_SUCCESS，失败时返回 OCI_#oc#oc#说明lsgetinfolsgetinfoVALID_HANDLE 或 OCI_ERROR。
...
| OCI_NLS_#oc#oc#说明lsgetinfolsgetinfoT_CURRENCY    | International 统通用符号 |
...
| OCI_NLS_L#oc#oc#说明lsgetinfolsgetinfoGUISTIC_NAME | 语言排序名称              |
| OCI_NLS_CALENDAR        | 日历名称                |
| OCI_NLS_WRIT#oc#oc#说明lsgetinfolsgetinfoG_DIR     | 语言写作方向              |
...
| OCI_NLS_NUMGROUP#oc#oc#说明lsgetinfolsgetinfoG     | 数字分组字段              |
| OCI_NLS_LISTSEP         | 列表分隔符               |
| OCI_NLS_MONDECIMAL      | 货币小数字符              |
| OCI_NLS_MONGROUP        | 货币组分隔符              |
| OCI_NLS_MONGROUP#oc#oc#说明lsgetinfolsgetinfoG     | 货币分组字段              |
| OCI_NLS_#oc#oc#说明lsgetinfolsgetinfoT_CURRENCYSEP | International 通用分隔符 |
