...
| item (IN)    | 从全球化支持环境变量中获取以下指定值之一： </br><ur><li>`OCI_NLS_CHARSET_ID`：`ub2` 数据类型的 `NLS_LANG` 字符集 ID。</li><li>`OCI_NLS_NCHARSET_ID`：`ub2` 数据类型的 `NLS_NCHAR` 字符集 ID。 </li></ur>   |
...
| errhp (IN/OUT) | 错误信息句柄。如果有错误，则记录在 `errhp` 中，并且该函数返回一个 `NULL` 指针。通过调用 [OCIErrorGet](../5.reference-function/7.miscellaneous-functions.md) 获取诊断信息。                                                                                                                                                                          |
