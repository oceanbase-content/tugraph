...
#docslug#/obci/obci/V1.1.0/zr8wnz
...
**返回值** ：成 功 返 回OCI_SUCCESS， 失 败 返 回OCI_ERROR， 出 现 警 告 信 息 返 回OCI_SUCCESS_WITH_INFO，超过行集末尾而无法获取数据返回OCI_NO_DATA。

**说明** ：下表展示了 OBCI支持的游标方向。
...
**函数功能** ：准备一条SQL语句，以便随后调用OCIStmtExecute来执行，在语句缓存已启用的情况下使用该函数。
...
| stmthp(OUT)  | 语句句柄。                                                   |
| errhp(IN)    | 错误信息句柄。                                                 |
| stmttext(IN) | 语句文本。其语义与 OCIStmtPrepare() 相同，所以字符串必须以 NULL 结尾。         |
| stmt_len(IN) | stmt 参数中 SQL 语句的长度。                                     |
| key(IN)      | 仅用于语句缓存，用于在语句缓存中搜索语句的键。如果传递了键，那么语句文本和其他参数将被忽略，并且搜索仅基于键。 |
