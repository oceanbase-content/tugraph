...
**函数功能** ：提取 C++ 生成的结果集中的行集。当执行一条查询以后，可以多次调用该函数来返回结果集中的所有行，直到该函数返回 OCI_NO_DATA。
...
**函数功能** ：准备一条 C++ 语句，以便随后调用 OCIStmtExecute 来执行。
...
| stmtp(IN/OUT) | 语句句柄                                                   |
| errhp(IN/OUT) | 错误信息句柄                                                 |
| key(IN)       | 仅对语句缓存有效。这是调用方传递的 C++ 字符串。如果在该语句中传递了 NULL 键，则不会对其进行标记。 |
