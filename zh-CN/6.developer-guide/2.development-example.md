...
如果您的数据库连接串信息是域名形式，可在 tnsnames.ora 文件中追加如下连接配置，通过 dblink 方式访问数据库，连接串的 dblink 直接写 "OB_SERVICE" 标识即可。
...
运行时，通过下述配置即可使用域名进行访问：

```bash
export TNS_ADMIN=   #设置tnsnames.ora的目录路径，不设置自动去程序当前目录查找
```
